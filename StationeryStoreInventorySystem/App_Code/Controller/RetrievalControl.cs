...
public class RetrievalControl
{
    ...
    {
        return context.Retrievals.Where(x => x.RetrievalStatus == "Pending" || x.RetrievalStatus == "InProgress").ToList();////
    }
    ...
    public List<RetrievalShortfallItem> UpdateRetrieval(int rId, Dictionary<string, int> retrievedData)
    {
        ...
        updateRetrievalStatus(rId, "InProgress");

        List<Disbursement> disbList = context.Disbursements.Where(x => x.RetrievalID == rId).ToList();
    ...
    public void updateRetrievalStatus(int rId, string status)
    {
    ...
    {
        updateRetrievalStatus(rId, "Closed");
